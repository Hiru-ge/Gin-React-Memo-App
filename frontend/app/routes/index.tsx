          ...
          { memos.map((memo) =>
            <div key={memo.id} className="bg-white rounded-lg shadow p-6" onClick={() => navigate(`/memos/${memo.id}`)}>
              ...
              <div className="mt-4 flex space-x-2">
                <button className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600" onClick={(e) => {e.stopPropagation(); navigate(`/memos/${memo.id}/edit`)}}>
                  編集
                </button>
                <button className="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600" onClick={(e) => {e.stopPropagation(); setDeletingMemo(memo)}}>
                  削除
                </button>
              </div>
            </div>
