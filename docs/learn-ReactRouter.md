...
export default function CreateMemo() {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button disabled={loading}>{loading ? "送信中..." : "作成"}</button>
    </form>
  );
}
```
...
```tsx
// 現在のURL: /memos/123
<Form method="post">{/* 送信先: /memos/123（現在のURL） */}</Form>
```
...
function FavoriteButton({ contact }: { contact: Contact }) {
  ...
  return (
    <fetcher.Form method="post" action="/contacts/favorite">
      ...
      />
      <button type="submit">{contact.favorite ? "★" : "☆"}</button>
    </fetcher.Form>
  );
}
```
...
export default function MemoDetail({ loaderData }: Route.ComponentProps) {
  ...
  return (
    <div>
      ...
      <button onClick={() => navigate(-1)}>戻る</button>
      <button onClick={() => navigate(`/memos/${memo.id}/edit`)}>編集</button>
    </div>
  );
}
```
  ...
  return (
    <div>
      <ul>
        ...
        {newTodo && <li className="opacity-50">{newTodo}</li>}
      </ul>
